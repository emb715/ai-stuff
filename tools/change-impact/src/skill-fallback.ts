/**
 * Skill fallback module for the change-impact tool.
 *
 * The tool loads `skills/change-impact-diagram/SKILL.md` as the LLM's system
 * prompt. When the skill file is not found at the expected path, this module
 * provides the skill content as a bundled string constant so the tool can
 * still operate without the file on disk.
 *
 * Bundling the content here keeps the tool self-contained: a missing file
 * degrades to an identical prompt rather than a hard failure.
 */

import { readFileSync } from "node:fs";
import { resolve } from "node:path";

/** Default path to the skill file, resolved relative to process.cwd(). */
export const DEFAULT_SKILL_PATH = "skills/change-impact-diagram/SKILL.md";

/**
 * Bundled copy of `skills/change-impact-diagram/SKILL.md`.
 *
 * Used as the fallback when the skill file cannot be read from disk. Kept in
 * sync with the source file; if the skill changes, regenerate this constant.
 */
export const SKILL_FALLBACK = `---
 ...
 *
 * Tries to read the skill file from `skillPath` (default
 * `skills/change-impact-diagram/SKILL.md`), resolved relative to `process.cwd()`.
 * If the file cannot be read (missing, unreadable, wrong encoding), returns the
 * bundled {@link SKILL_FALLBACK} so callers always get a usable prompt.
 *
 * @param skillPath - Optional path to the skill file, relative to `process.cwd()`.
 * @returns The skill file content, or the bundled fallback if reading fails.
 */
export function loadSkill(skillPath: string = DEFAULT_SKILL_PATH): string {
  const resolved = resolve(process.cwd(), skillPath);
  try {
