...
- **Structured extraction over free-form reads.** Round 0 pulls from specific PRD sections (Scope In, Architecture, Constraints, Acceptance Criteria) rather than a free-form read. This prevents both under-enumeration (missing a constraint) and over-enumeration (inventing a requirement the doc doesn't state). Added after a session where free-form extraction produced 16 items but missed shared root-cause grouping.
...
- Post-amendment propagation was a real failure in the validating session: a spec's Task was amended to use `prisma.member.findMany` but the Test Plan still mocked `prisma.user.findMany`. The per-requirement rubric passed (the requirement was now covered) but the spec was internally inconsistent. The post-amendment propagation check (round step 7) was added to catch this. If this check is skipped, stale test mocks are the predictable failure.
- Shared-root-cause detection was missing in the validating session: 3 requirements were all `partial` for the same reason and were resolved one at a time across 3 rounds. The Round 0 post-classification scan was added to group them. If the loop seems to be re-solving the same problem, check whether the scan ran.