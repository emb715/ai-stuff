...
**Read first block:** list the plan file and any ADR/doc that is the source of truth. Pull paths from selected plan source + session context.

**Context resume:** 2–5 lines max. Current readiness verdict, the critical blockers only — no history, no explanation. Extracted from plan source/session, not re-derived.
