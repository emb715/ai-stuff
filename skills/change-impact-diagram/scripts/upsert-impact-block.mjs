#!/usr/bin/env node
// Upsert a change-impact block into a PR description without touching any
// text outside the markers. Usage:
//   node upsert-impact-block.mjs <pr-number> <block-file>
...
const startMarker = '<!-- change-impact:start -->'
const endMarker = '<!-- change-impact:end -->'

const [prNumber, blockFile] = process.argv.slice(2)
if (!prNumber || !blockFile) {
	console.error('usage: upsert-impact-block.mjs <pr-number> <block-file>')
	process.exit(1)
}
